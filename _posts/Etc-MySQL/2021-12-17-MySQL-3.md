...
![1]({{ site.images }}/assets/posts/Etc/MySQL/lecture-3/1.webp){: width="100%" height="100%"}
...
테이블을 생성하는 방법은 <code>CREATE TABLE `{데이터베이스 이름}`.`{테이블 이름}`({열의 종류});</code>의 구조로 명령어를 실행합니다.
...
테이블 조회는 <code>DESC `{데이터베이스 이름}`.`{테이블 이름}`;</code>의 구조로 명령어를 실행합니다.
...
테이블 이름 변경은 <code>ALTER TABLE `{데이터베이스 이름}`.`{기존 테이블 이름}` RENAME `{데이터베이스 이름}`.`{새로운 테이블 이름}`;</code>의 구조로 명령어를 실행합니다.
...
테이블에 코멘트를 추가하는 방법은 <code>ALTER TABLE `{데이터베이스 이름}`.`{테이블 이름}` COMMENT="{코멘트 내용}";</code>의 구조로 명령어를 실행합니다.
...
테이블에 열을 추가하는 방법은 <code>ALTER TABLE `{데이터베이스 이름}`.`{테이블 이름}` ADD `{열 이름}` `{열 속성}`;</code>의 구조로 명령어를 실행합니다.
...
`콤마(,)`를 활용하여 한 번에 여러 개의 열을 추가할 수 있습니다.
...
테이블에 열을 수정하는 방법은 <code>ALTER TABLE `{데이터베이스 이름}`.`{테이블 이름}` CHANGE `{열 이름}` `{열의 새로운 이름} `{열의 새로운 속성}`;</code>의 구조로 명령어를 실행합니다.
...
테이블에 열을 수정하는 방법은 <code>ALTER TABLE `{데이터베이스 이름}`.`{테이블 이름}` MODIFY `{열 이름}` `{열의 새로운 속성}`;</code>의 구조로 명령어를 실행합니다.
...
`콤마(,)`를 활용하여 한 번에 여러 개의 열을 수정할 수 있습니다.
...
테이블에 열을 삭제하는 방법은 <code>ALTER TABLE `{데이터베이스 이름}`.`{테이블 이름}` DROP COLUMN `{열 이름}`;</code>의 구조로 명령어를 실행합니다.
...
테이블 이름 변경은 <code>DROP TABLE `{데이터베이스 이름}`.`{테이블 이름}`;</code>의 구조로 명령어를 실행합니다.
